...
from ml_lax_spectral_family.diagnostics import relative_matrix_error, relative_residual, singular_values
from ml_lax_spectral_family.models import ComplexLinearMapNet
from ml_lax_spectral_family.s2 import apply_q, exact_q_matrix, sample_offshell_batch
...
def support_violation_fraction(q: np.ndarray, eps: float = 1.0e-30) -> float:
    allowed = np.zeros_like(q)
    allowed[..., 0, 0] = q[..., 0, 0]
    allowed[..., 1, 1] = q[..., 1, 1]
    forbidden = q - allowed
    return float(np.linalg.norm(forbidden) / (np.linalg.norm(q) + eps))


def active_entry_relation_error(q: np.ndarray, eps: float = 1.0e-30) -> float:
    numerator = np.linalg.norm(q[..., 0, 0] + q[..., 1, 1])
    denominator = np.linalg.norm(q[..., 0, 0]) + np.linalg.norm(q[..., 1, 1]) + eps
...
def main() -> None:
    ...
    support_error = support_violation_fraction(q_pred)
    relation_error = active_entry_relation_error(q_pred)
    ...
    metrics = {
        ...
        "q_forbidden_entry_fraction": support_error,
        "q_active_entry_relation_error": relation_error,
